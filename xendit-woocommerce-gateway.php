...
/*
Plugin Name: WooCommerce - Xendit Cards & Subscriptions
Plugin URI: https://wordpress.org/plugins/xendit-woocommerce-gateway/
Description: Accept Credit/Debit cards with Xendit. Works with WooCommerce & Subscriptions. This plugin will not be updated in the future and will be <strong>deprecated</strong> soon. Please update or install <a href='https://wordpress.org/plugins/woo-xendit-virtual-accounts/'>WooCommerce - Xendit</a> plugin to accept single transaction and subscription with Credit Cards and Online Debit Card without breaking your current payment flow.
Version: 1.5.2
Author: Xendit
 ...
 */
define('WC_XENDIT_VERSION', '1.5.3');
...
if (! class_exists('WC_Xendit')) :
    ...
    class WC_Xendit
    {
        ...
        /**
         * Endpoint to fetch message for news, announcement, etc.
         * @var string
         */
        public $message_endpoint = 'https://tpi.xendit.co/messages';

        /**
         * Default message that will be retrieved from cache
         * @var string
         */
        public $message = "WooCommerce - Xendit Cards & Subscriptions plugin will not be updated in the future and will be <strong>deprecated</strong> soon. Please update or install <a href='https://wordpress.org/plugins/woo-xendit-virtual-accounts/'>WooCommerce - Xendit</a> plugin to accept single transaction and subscription with Credit Cards and Online Debit Card without breaking your current payment flow.";
        ...
        protected function __construct()
        {
            ...
            add_action('admin_notices', array( $this, 'show_deprecated_message' ));
        }
        ...
        public function init()
        {
            ...
            require_once(dirname(__FILE__) . '/includes/enum/class-logdna-level.php');

            require_once(dirname(__FILE__) . '/includes/class-wc-xendit-logger.php');
            require_once(dirname(__FILE__) . '/includes/class-wc-xendit-logdna.php');
            require_once(dirname(__FILE__) . '/includes/class-wc-xendit-helper.php');
            include_once(dirname(__FILE__) . '/includes/class-wc-xendit-api.php');

            // Init the gateway itself
            $this->init_gateways();
            require_once(dirname(__FILE__) . '/includes/class-wc-xendit-redirect-handler.php');
        ...
        public function show_deprecated_message()
        {
            if (is_plugin_active(plugin_basename(__FILE__))) {
                ...
                if (false === $cache_message) {
                    ...
                    $cache_message = wp_remote_get($this->message_endpoint . '?key=woocommerce_cc_deprecation_warning', $args);
                    ...
                    $this->message = $cache_message;
                }

                $url = admin_url() . 'admin.php?page=wc-settings&tab=checkout&section=xendit';
                $html = '<div class="notice notice-warning">
                            <p>' . str_replace('{xendit-cc-plugin-url}', $url, $this->message) . '</p>
                        </div>';
                echo $html;
            }
        }
